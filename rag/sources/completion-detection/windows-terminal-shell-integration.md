---
title: "Shell integration in the Windows Terminal (Microsoft Learn)"
source_url: "https://learn.microsoft.com/en-us/windows/terminal/tutorials/shell-integration"
source_type: docs
fetched: 2026-06-01
topic: completion-detection
tags: ["osc-133", "windows-terminal", "bash", "powershell", "ps0-ps1", "wsl"]
summary: "Windows TerminalのOSC 133マーク対応と、bash(PS0/PS1/PS2)・PowerShell・CMDでマーカーを注入する具体的プロンプト設定。"
relevance: "WSL2上の対象環境そのもの。bashのPS0=133;C/PS1=133;A..B注入の実コードがあり、自前でマーカーを仕込む方式の雛形になる。"
chars: 17019
---
...
Add to plan
[Edit](https://github.com/MicrosoftDocs/terminal/blob/main/TerminalDocs/tutorials/shell-integration.md)
...
* `OSC 133 ; A ST` ("*FTCS\_PROMPT*") - The start of a prompt.
* `OSC 133 ; B ST` ("*FTCS\_COMMAND\_START*") - The start of a commandline (READ: the end of the prompt).
* `OSC 133 ; C ST` ("*FTCS\_COMMAND\_EXECUTED*") - The start of the command output / the end of the commandline.
* `OSC 133 ; D ; <ExitCode> ST` ("*FTCS\_COMMAND\_FINISHED*") - the end of a command. `ExitCode` If `ExitCode` is provided, then the Terminal will treat `0` as "success" and anything else as an error. If omitted, the terminal will just leave the mark the default color.
...
If you've never changed your PowerShell prompt before, you should check out [about\_Prompts](/en-us/powershell/module/microsoft.powershell.core/about/about_prompts) first.
...
Add the following to your [PowerShell profile](/en-us/powershell/module/microsoft.powershell.core/about/about_profiles):
...
(For more info, see the [Suggestions documentation](../customize-settings/actions#open-suggestions-menu))

## Additional resources

* [`autoMarkPrompts`](../customize-settings/profile-advanced#automatically-add-scroll-marks)
* [`showMarksOnScrollbar`](../customize-settings/profile-advanced#show-marks-on-scrollbar)
* [`showSuggestions`](../customize-settings/actions#open-suggestions-menu)
...
![](https://learn.microsoft.com/windows/terminal/images/terminal.svg)

Windows Terminal

Open a documentation issue
[Provide product feedback](https://github.com/Microsoft/terminal/issues)
