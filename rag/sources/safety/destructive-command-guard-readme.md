...
source_url: "https://raw.githubusercontent.com/Dicklesworthstone/destructive_command_guard/main/README.md"
source_type: github_readme
fetched: 2026-06-01
topic: safety
tags: ["destructive-command", "whitelist-first", "ai-agent-guard", "rm-rf", "git-reset-hard", "confirmation-gate", "hook"]
summary: "AIエージェントが実行する前に rm -rf / git reset --hard / DROP TABLE 等の破壊的コマンドを遮断するフック。安全パターン先評価のホワイトリスト先行設計、サブミリ秒判定。"
relevance: "PTYへkeystrokeを流す前段の破壊的コマンド確認パターンの実装例。ホワイトリスト先行・カテゴリ別ゲートの具体設計を、我々のsend前ガードに転用できる。"
chars: 105528
---
...
**The Problem**: AI coding agents (Claude, Codex, Gemini, Copilot, etc.) occasionally run catastrophic commands like `git reset --hard`, `rm -rf ./src`, or `DROP TABLE users`ŌĆödestroying hours of uncommitted work in seconds.
...
| **Sub-Millisecond Latency** | SIMD-accelerated filteringŌĆöyou won't notice it's there |
...
recorded in JSON output and logs ŌĆö it does not directly change rule evaluation.
...
```toml
# Trust Claude Code more ŌĆö wider allowlist, fewer packs
[agents.claude-code]
trust_level = "high"
additional_allowlist = ["npm run build", "cargo test"]
disabled_packs = ["kubernetes"]

# Restrict unknown agents ŌĆö extra rules, no allowlist bypass
[agents.unknown]
trust_level = "low"
...
Ōöé Tier 1: Trigger Ōöé ŌöĆŌöĆŌöĆ No match ŌöĆŌöĆŌ¢║ ALLOW (fast path, <100╬╝s)
Ōöé   (RegexSet)    Ōöé
ŌööŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌö¼ŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöś
         Ōöé Match
         Ō¢╝
ŌöīŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöÉ
Ōöé Tier 2: Extract Ōöé ŌöĆŌöĆŌöĆ Error/Timeout ŌöĆŌöĆŌ¢║ ALLOW + fallback check
Ōöé   (<1ms)        Ōöé
ŌööŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌö¼ŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöś
         Ōöé Success
         Ō¢╝
ŌöīŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöÉ
Ōöé Tier 3: AST     Ōöé ŌöĆŌöĆŌöĆ No match ŌöĆŌöĆŌ¢║ ALLOW
Ōöé   (<5ms)        Ōöé ŌöĆŌöĆŌöĆ Match ŌöĆŌöĆŌ¢║ BLOCK
ŌööŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöś
```

**Tier 1: Trigger Detection** (<100╬╝s)
...
Commands without any trigger patterns skip directly to ALLOWŌĆöno further processing needed.

**Tier 2: Content Extraction** (<1ms)

For commands that trigger, extract the actual content to be evaluated:

- **Heredocs**: `cat <<EOF ... EOF` ŌåÆ extracts body between delimiters
- **Here-strings**: `cat <<< "content"` ŌåÆ extracts quoted content
- **Inline scripts**: `python -c "code"` ŌåÆ extracts the code argument
...
This means you can set organization defaults in `/etc/dcg/config.toml`, personal preferences in `~/.config/dcg/config.toml`, and project-specific overrides in `.dcg.toml`ŌĆöeach layer only needs to specify the settings that differ from defaults.
...
Evaluation Trace:
  [  0.8╬╝s] Quick reject: passed (contains 'git')
  [  2.1╬╝s] Normalize: no changes
  [  5.3╬╝s] Safe patterns: no match (checked 34 patterns)
  [ 12.7╬╝s] Destructive patterns: MATCH at pattern 'reset-hard'
  [ 12.9╬╝s] Total time: 12.9╬╝s
...
AI coding agents routinely get stuck when `git pull --rebase` fails partway ŌĆö unstaged-changes errors, stash-pop conflicts, interrupted rebases. The documented recovery path is almost always `git checkout -- .` or `git restore <paths>`, both of which dcg hard-blocks (`core.git:checkout-discard`, `core.git:restore-worktree`). Agents then have to stop and ask a human to run the command manually.
...
1. **Active rebase state (automatic, zero-config).** When `.git/rebase-merge/` or `.git/rebase-apply/` exists, a rebase is in progress and the discard operations *are* the documented recovery path. dcg detects this state and converts the deny into an allow with a `[dcg] Allowing ... ŌåÆ rebase-recovery mode` note on stderr. No permit needed.
...
2. **Explicit permit cookie (opt-in, short-lived).** When the rebase already finished but the worktree is still messy (e.g. after a bad `git stash pop`), run:
   ...
   This writes a timestamp to `.dcg/rebase-recovery-permit` at the repo root. For the next N seconds (or until the first matching allow, whichever comes first), `git checkout -- <path>` and `git restore <paths>` are allowed. The permit is **single-shot** ŌĆö one successful allow consumes it ŌĆö so it can't silently unblock later unrelated commands within the TTL.
...
- name: Build
  run: |                    # ŌåÉ Extracted
    ...
    npm run build
  env:
    NODE_ENV: production    # ŌåÉ Skipped (not executable)
```

```dockerfile
# Dockerfile - only RUN instructions
FROM node:18
COPY . /app                 # ŌåÉ Skipped
RUN npm install             # ŌåÉ Extracted
RUN ["node", "server.js"]   # ŌåÉ Extracted (exec form)
ENV PORT=3000               # ŌåÉ Skipped
```

```makefile
# Makefile - tab-indented lines under targets
build:
	npm install             # ŌåÉ Extracted (recipe line)
	npm run build           # ŌåÉ Extracted
SOURCES = $(wildcard *.js)  # ŌåÉ Skipped (variable assignment)
```
...
Ōöé   Claude / Codex / Gemini / Copilot / Cursor / Hermes hooks      Ōöé
Ōöé                                                                  Ōöé
Ōöé  User: "delete the build artifacts"                             Ōöé
Ōöé  Agent: executes `rm -rf ./build`                               Ōöé
Ōöé                                                                  Ōöé
...
Ōöé  Ōöé    Parse     ŌöéŌöĆŌöĆŌöĆŌ¢ČŌöé  Normalize   ŌöéŌöĆŌöĆŌöĆŌ¢ČŌöé Quick Reject Ōöé       Ōöé
Ōöé  Ōöé    JSON      Ōöé    Ōöé   Command    Ōöé    Ōöé   Filter     Ōöé       Ōöé
...
Ōöé  Ōöé                   Pattern Matching                        Ōöé   Ōöé
Ōöé  Ōöé                                                           Ōöé   Ōöé
Ōöé  Ōöé   1. Check SAFE_PATTERNS (whitelist) ŌöĆŌöĆŌ¢Č Allow if match  Ōöé   Ōöé
Ōöé  Ōöé   2. Check DESTRUCTIVE_PATTERNS ŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌ¢Č Deny if match    Ōöé   Ōöé
Ōöé  Ōöé   3. No match ŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌ¢Č Allow (default)  Ōöé   Ōöé
Ōöé  Ōöé                                                           Ōöé   Ōöé
...
ŌööŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌö¼ŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöś
                      Ōöé
                      Ō¢╝ stdout: JSON deny / empty allow
                        stderr: rich human output / Codex deny reason
ŌöīŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöĆŌöÉ
Ōöé   Claude / Codex / Gemini / Copilot / Cursor / Hermes hooks      Ōöé
Ōöé                                                                  Ōöé
Ōöé  If denied: Shows block message, does NOT execute command       Ōöé
Ōöé  If allowed: Proceeds with command execution                    Ōöé
Ōöé                                                                  Ōöé
...
```
git checkout -b feature    ŌåÆ  Matches SAFE "checkout-new-branch"  ŌåÆ  ALLOW
git checkout -- file.txt   ŌåÆ  No safe match, matches DESTRUCTIVE  ŌåÆ  DENY
```
...
```bash
/usr/bin/git reset --hard          # Blocked Ō£ō
/usr/local/bin/git checkout -- .   # Blocked Ō£ō
/bin/rm -rf /home/user             # Blocked Ō£ō
```
...
```bash
git push --force                  # Blocked Ō£ō
git push origin main --force      # Blocked Ō£ō
git push --force origin main      # Blocked Ō£ō
git push -f                       # Blocked Ō£ō
git push --force-with-lease       # Allowed Ō£ō (safe alternative)
```
...
```bash
git restore --staged file.txt           # Allowed Ō£ō (unstaging only)
git restore -S file.txt                 # Allowed Ō£ō (short flag)
...
3. **Automatic optimization**: Pattern authors don't need to think about engine selectionŌĆödcg chooses optimally
...
|------|------|--------|---------------|-------------|
| 0 | Quick reject | < 1╬╝s | > 5╬╝s | > 50╬╝s |
| 1 | Fast path | < 75╬╝s | > 150╬╝s | > 500╬╝s |
| 2 | Pattern match | < 100╬╝s | > 250╬╝s | > 1ms |
| 3 | Heredoc trigger | < 5╬╝s | > 10╬╝s | > 100╬╝s |
| 4 | Heredoc extract | < 200╬╝s | > 500╬╝s | > 2ms |
| 5 | Language detect | < 20╬╝s | > 50╬╝s | > 200╬╝s |
...
```
[WARN] Performance budget exceeded: Tier 2 (safe patterns) took 1.2ms (panic threshold: 500╬╝s)
...
```
Evaluation Trace:
  [  0.3╬╝s] Tier 0: Quick reject (PASS - below 1╬╝s target)
  [  8.7╬╝s] Tier 1: Fast path (PASS - below 75╬╝s target)
  [ 15.2╬╝s] Tier 2: Pattern match (PASS - below 100╬╝s target)
  [ 15.4╬╝s] Total: 15.4╬╝s (PASS - below 5ms target)
```

### Keyword-Based Pack Pre-filtering

Before expensive regex matching, dcg uses a multi-level keyword filtering system to quickly skip irrelevant packs. This is critical for performanceŌĆöwith 50+ packs available, checking every pattern against every command would be prohibitively slow.
...
dcg doesn't just block commandsŌĆöit provides actionable guidance to help users make safer choices. The suggestion system generates context-aware recommendations based on the specific command that was blocked.
...
```bash
dcg setup               # Interactive ŌĆö prompts before modifying RC files
dcg setup --shell-check # Non-interactive ŌĆö adds the check automatically
```
...
if command -v dcg &>/dev/null && command -v jq &>/dev/null; then
  if [ -f "$HOME/.claude/settings.json" ] && \
    ...
    printf '\033[1;33m[dcg] Hook missing from ~/.claude/settings.json ŌĆö run: dcg install\033[0m\n'
  fi
fi
```
