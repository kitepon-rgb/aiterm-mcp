---
title: "mcp-server-commands (g0t4) README"
source_url: "https://raw.githubusercontent.com/g0t4/mcp-server-commands/master/README.md"
source_type: github_readme
fetched: 2026-06-01
topic: prior-art
tags: ["mcp", "one-shot", "run_process", "command_line-vs-argv", "stdin", "approval", "safety", "stdio"]
summary: "単一ツールrun_processでコマンドを実行するMCPサーバ。command_line(シェル経由)とargv(シェル解釈なし)を排他で選択、stdin対応。"
relevance: "単発・非永続モデルの代表。command_line vs argv(シェル注入面の分離)とapprove-once承認フローは、我々がsendで生文字列を流す際の安全設計とインジェクション面の対比になる。完了=プロセス終了の素朴境界の基準点。"
chars: 5587
---
...
1. **`command_line`** (string) â€” Executed via the system's default shell (just like typing into `bash`/`fish`/`pwsh`/etc). Shell features like pipes, redirects, and variable expansion all work.
2. **`argv`** (string array) â€” Direct executable invocation. `argv[0]` is the executable, the rest are arguments. No shell interpretation.
