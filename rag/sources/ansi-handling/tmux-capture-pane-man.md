---
title: "tmux(1) man page - capture-pane (-e/-p/-C/-N/-J/-S/-E)"
source_url: "https://man7.org/linux/man-pages/man1/tmux.1.html"
source_type: man
fetched: 2026-06-01
topic: ansi-handling
tags: ["tmux", "capture-pane", "escape-sequences", "scrollback", "backend", "man"]
summary: "tmuxの公式man。capture-paneの-e(エスケープ付き)/-p(stdout)/-J(折返し結合)/-S -E(行範囲)等の正確な仕様。"
relevance: "我々のtmuxバックエンドで画面を読む唯一の窓口の正典。-e有無で生シーケンスかプレーンか、-Sでscrollback範囲をどう取るかの一次仕様。"
chars: 234635
---
...
|  |
| --- |
| [NAME](#NAME) | [SYNOPSIS](#SYNOPSIS) | [DESCRIPTION](#DESCRIPTION) | [DEFAULT KEY BINDINGS](#DEFAULT_KEY_BINDINGS) | [COMMAND PARSING AND EXECUTION](#COMMAND_PARSING_AND_EXECUTION) | [PARSING SYNTAX](#PARSING_SYNTAX) | [COMMANDS](#COMMANDS) | [CLIENTS AND SESSIONS](#CLIENTS_AND_SESSIONS) | [WINDOWS AND PANES](#WINDOWS_AND_PANES) | [KEY BINDINGS](#KEY_BINDINGS) | [OPTIONS](#OPTIONS) | [HOOKS](#HOOKS) | [MOUSE SUPPORT](#MOUSE_SUPPORT) | [FORMATS](#FORMATS) | [STYLES](#STYLES) | [NAMES AND TITLES](#NAMES_AND_TITLES) | [GLOBAL AND SESSION ENVIRONMENT](#GLOBAL_AND_SESSION_ENVIRONMENT) | [STATUS LINE](#STATUS_LINE) | [BUFFERS](#BUFFERS) | [MISCELLANEOUS](#MISCELLANEOUS) | [EXIT MESSAGES](#EXIT_MESSAGES) | [TERMINFO EXTENSIONS](#TERMINFO_EXTENSIONS) | [CONTROL MODE](#CONTROL_MODE) | [ENVIRONMENT](#ENVIRONMENT) | [FILES](#FILES) | [EXAMPLES](#EXAMPLES) | [SEE ALSO](#SEE_ALSO) | [AUTHORS](#AUTHORS) | [COLOPHON](#COLOPHON) |
|  |  |
...
## DEFAULT KEY BINDINGS         [top](#top_of_page)
...
```
       ...
       The default command key bindings are:
             ...
             M-1 to M-7  Arrange panes in one of the seven preset
                         ...
                         main-vertical, main-vertical-mirrored, or tiled.
...
## COMMAND PARSING AND EXECUTION         [top](#top_of_page)
...
## PARSING SYNTAX         [top](#top_of_page)
...
```
       ...
       target-session, target-window or target-pane each denote the
       ‘type’ of target the command needs to work on.  If a target is not
       explicitly qualified using ‘:’ and ‘.’, tmux will pick what seems
       to be the best choice available.  For example, if a target-pane of
       ‘1’ is used with a current window that has only one pane, tmux
       knows that the ‘1’ cannot mean a pane, so it will look for the
       active pane in window 1, or the active pane in the current window
       in session 1.  Only if none of these are present will it report an
       error.  If it is important that a specific pane, window or session
       always be used, such as from a script, the target should be fully
       qualified (for example a target-pane of ‘-t:.1’), or pane, window
       or session IDs should be used.
...
## CLIENTS AND SESSIONS         [top](#top_of_page)
...
```
       ...
       list-clients [-r] [-F format] [-f filter] [-O sort-order] [-t
               target-session]
                     (alias: lsc)
               ...
               section.  -O specifies the sort order: one of ‘name’,
               ‘size’, ‘creation’ (time), or ‘activity’ (time).  -r
               reverses the sort order.  If target-session is specified,
       ...
       list-sessions [-r] [-F format] [-f filter] [-O sort-order]
               ...
               which the filter is true are shown.  See the “FORMATS”
               section.  -O specifies the sort order: one of ‘index’,
               ‘name’, ‘creation’ (time), or ‘activity’ (time).  -r
       ...
       refresh-client [-cDlLRSU] [-A pane:state] [-B name:what:format]
               [-C size] [-f flags] [-r pane:report] [-t target-client]
               [adjustment]
                     (alias: refresh)
       ...
       switch-client [-ElnprZ] [-c target-client] [-O sort-order] [-t
               ...
               respectively.  -O may be used with -n and -p to specify
               the field to sort on: one of ‘name’, ‘size’, ‘creation’
               (time), or ‘activity’ (time).  -r toggles the client
               read-only and ignore-size flags (see the attach-session
               command).
               ...
               looked up using key-table.  After that key, the client is
               returned to its default key table (normally root).  This
...
## WINDOWS AND PANES         [top](#top_of_page)
...
```
       ...
       scroll-exit-on
               Turn on exiting copy mode when scrolling to the end of the
               buffer.

       scroll-exit-off
               Turn off exiting copy mode when scrolling to the end of
               the buffer.

       scroll-exit-toggle
               Toggle exiting copy mode when scrolling to the end of the
               buffer.
       ...
       choose-client [-NryZ] [-F format] [-f filter] [-K key-format] [-O
               ...
               -O specifies the initial sort order: one of ‘name’,
               ‘size’, ‘creation’ (time), or ‘activity’ (time).  -r
       ...
       choose-tree [-GNrswyZ] [-F format] [-f filter] [-K key-format] [-O
               ...
               The following keys may be used in tree mode:
                     ...
                     S-Up         Swap the current window with the
                                              previous one
                     S-Down       Swap the current window with the next
                                              one
               ...
               -O specifies the initial sort order: one of ‘index’,
               ‘name’, or ‘activity’ (time).  -r reverses the sort order.
       ...
       customize-mode [-NZ] [-F format] [-f filter] [-t target-pane]
               ...
               may be used in customize mode:
                     ...
                     Key          Function
                     Enter        Set pane, window, session or global
                                              option value
                     Up           Select previous item
                     Down         Select next item
                     +            Expand selected item
                     ...
                     w            Set window option value, if option is
                                              for pane and window
                     d            Set an option or key to the default
                     D            Set tagged options and tagged keys to
                                              the default
                     u            Unset an option (set to default value
                                              if global) or unbind a key
                     U            Unset tagged options and unbind tagged
                                              keys
                     C-s          Search by name
                     n            Repeat last search forwards
                     N            Repeat last search backwards
       ...
       list-panes [-ars] [-F format] [-f filter] [-O sort-order] [-t
               target]
                     (alias: lsp)
               ...
               filter is true are shown.  See the “FORMATS” section.  -O
               specifies the sort order: one of ‘name’ (title), ‘index’,
               ‘size’ (area), ‘creation’ (time), or ‘activity’ (time).
               -r reverses the sort order.

       list-windows [-ar] [-F format] [-f filter] [-O sort-order] [-t
               target-session]
                     (alias: lsw)
               ...
               are shown.  See the “FORMATS” section.  -O specifies the
               sort order: one of ‘index’, ‘name’, ‘size’ (area),
               ‘creation’ (time), or ‘activity’ (time).  -r reverses the
               sort order.
       ...
       new-window [-abdkPS] [-c start-directory] [-e environment] [-F
               ...
               shell startup files or by the -e option.
       ...
       new-pane [-bdefhIkPvZ] [-c start-directory] [-e environment] [-F
               format] [-l size] [-m message] [-p percentage] [-R
               inactive-border-style] [-s style] [-S active-border-style]
               [-t target-pane] [shell-command [argument ...]]
                     (alias: newp)
               Create a new pane.  The new pane is created by splitting
               target-pane.  If -d is given, the session does not make
               the new pane the current pane.  -Z zooms if the window is
               not zoomed, or keeps it zoomed if already zoomed.  -s sets
               the style for the pane content.  -S sets the border style
               when the pane is active and -R sets the border style when
               the pane is inactive (see “STYLES”).

               -h does a horizontal split and -v a vertical split; if
               neither is specified, -v is assumed.  The -l option
               specifies the size of the new pane in lines (for vertical
               split) or in columns (for horizontal split); size may be
               followed by ‘%’ to specify a percentage of the available
               space.  -p is a shorthand option for this.  The -b option
               causes the new pane to be created to the left of or above
               target-pane.  The -f option creates a new pane spanning
               the full window height (with -h) or full window width
               (with -v), instead of splitting the active pane.

               -k keeps the pane open after the optional shell-command
               exits and waits for a key to be pressed before closing it.
               The message shown is controlled by the
               remain-on-exit-format option.  -m message is equivalent to
               -k but also sets the remain-on-exit-format option for this
               pane to message.

               An empty shell-command ('') will create a pane with no
               command running in it.  The -I flag (if shell-command is
               not specified or empty) will create an empty pane and
       ...
       split-window [-bdefhIkPvZ] [-c start-directory] [-e environment]
               [-F format] [-l size] [-m message] [-p percentage] [-R
               inactive-border-style] [-s style] [-S active-border-style]
               [-t target-pane] [shell-command [argument ...]]
                     (alias: splitw)
               Creates a new pane by splitting target-pane.  Shares
               behavior with new-pane.

               See new-pane for more details.
...
```
       ...
       list-keys [-1aNr] [-F format] [-O sort-order] [-P prefix-string]
               [-T key-table] [key]
                     (alias: lsk)
               List key bindings.  -F specifies the format of each line.
               See the “FORMATS” section.  -T specifies a key-table to
               list from.  -1 lists only the first matching key.  -O
               specifies the sort order: one of ‘key’, ‘modifier’, ‘name’
               (table name).  -r reverses the sort order.

               If no format is given, there are two forms: the default
               lists keys as bind-key commands; -N lists only keys with
               attached notes and shows only the key and note for each
               key.

               With the default form, all key tables are listed unless
               specified otherwise.
               ...
               key.  -a lists the command for keys that do not have a
...
```
       ...
       codepoint-widths[] string
               ...
               may be a UTF-8 character, an identifier of the form
               ‘U+number’ where the number is a hexadecimal number, or a
               range of the form ‘U+number-U+number’.
       ...
       get-clipboard [both | request | buffer | off]
               ...
               application; both is the same as request but also creates
               a paste buffer.
       ...
       message-format string
               ...
               The special placeholder ‘#{message}’ expands to the
               interactive prompt or message text and ‘#{command_prompt}’
               is set to 1 when the prompt is in command mode (vi).
               Style directives like fill, align, and width may be used
               in the format string.  The default uses a conditional to
               select between message-style and message-command-style.
       ...
       message-style style
               ...
               and for the command prompt.  The message is drawn on top
               of the existing status line.  A width attribute (a fixed
               number of columns or a percentage such as ‘50%’)
               constrains the prompt area width and an align attribute
               (left, centre, right) sets its horizontal position.  When
               the width is less than the full terminal width, the normal
               status bar content remains visible around the prompt area.
               The fill attribute is used by the default message-format
               to clear the background.  For how to specify style, see
               the “STYLES” section.
       ...
       status-justify [left | centre | right | absolute-centre]
               ...
               relative centre of the available free space;
       ...
       copy-mode-current-line-number-style style
               Set style of current line number in copy mode.  For how to
               specify style, see the “STYLES” section.

       copy-mode-line-number-style style
               Set style of line numbers in copy mode.  For how to
               specify style, see the “STYLES” section.

       copy-mode-line-numbers [off | default | absolute | relative |
               hybrid]
               Show line numbers in copy mode.  off hides line numbers,
               default shows line numbers matching the copy mode position
               indicator and goto-line, absolute shows absolute line
               numbers, relative shows line numbers relative to the
               cursor, and hybrid shows the current line number as
               absolute and other line numbers as relative.  With off or
               default, the position indicator and goto-line use the same
               numbering; with absolute, relative and hybrid, they use
       ...
       tree-mode-preview-style style
               Set the style of the preview indicator in tree mode.  For
               how to specify style, see the “STYLES” section.
       ...
       remain-on-exit [on | off | failed | key]
               ...
               when the program exit status is not zero.  If set to key,
               the pane stays open and closes when a key is pressed.  The
               pane may be reactivated with the respawn-pane command.
...
```
       ...
       Within the ‘W:’ loop, user options (‘@name’) from neighboring
       windows are available with a ‘next_’ or ‘prev_’ prefix, for
       example a user option ‘@color’ on the next window is available as
       ‘next_@color’.
       ...
       copy_position                   Position shown in the copy mode
                                       indicator
       copy_position_limit             Limit shown in the copy mode
                                       indicator
       ...
       next_session_id                 Unique session ID for next new
                                       session
       next_window_active              1 if next window in W: loop is
                                       active
       ...
       pane_pb_state                   Pane progress bar state, one of
                                       hidden, normal, error,
                                       indeterminate, paused (can be set
                                       by application)
       pane_pb_progress                Pane progress bar progress
       ...
       pane_floating_flag              1 if pane is floating
       pid                             Server PID
       prev_window_active              1 if previous window in W: loop is
                                       active
       prev_window_index               Index of previous window in W:
                                       loop
...
```
       ...
       width=N
               Set the width of the styled area.  N may be a column count
               or a percentage (for example ‘50%’).
       ...
       range=left, range=right, range=session|X, range=window|X,
               range=pane|X, range=user|X, range=control|N, norange
               ...
               range=control|N is a set of ranges for users to define
               custom behavior.  ‘N’ can be 0 through 9.  When a mouse
               event occurs in the range=control|N range, the ‘ControlN’
               key binding is triggered.  See “MOUSE SUPPORT” for
               details.
...
## NAMES AND TITLES         [top](#top_of_page)
...
## GLOBAL AND SESSION ENVIRONMENT         [top](#top_of_page)
...
```
       ...
       command-prompt [-1bCeFiklN] [-I inputs] [-p prompts] [-t
               ...
               If -I is present, inputs is a comma-separated list of the
               initial text for each prompt.  If -p is given, prompts is
               a comma-separated list of prompts which are displayed in
               order; otherwise a single prompt is displayed, constructed
               from template if it is present, or ‘:’ if not.  -l
               disables splitting of inputs and prompts at commas and
               ...
               makes the prompt only accept numeric key presses.  If -C
               given, panes will continue to be updated while the prompt
               is displayed.  -i executes the command every time the
               prompt input changes instead of when the user exits the
               command prompt.  -e makes BSpace cancel an empty prompt.
       ...
       display-popup [-BCEkN] [-b border-lines] [-c target-client] [-d
               ...
               box drawn over the top of any panes.  If the command is
               run inside an existing popup, that popup is modified.
               Only the -b, -B, -C, -E, -EE, -K, -N, -s, and -S options
               are accepted in this case; all others are ignored.
...
```
       ...
       choose-buffer [-NryZ] [-F format] [-f filter] [-K key-format] [-O
               ...
               -O specifies the initial sort order: one of ‘creation’
               (time), ‘name’, or ‘size’.  -r reverses the sort order.
               -f specifies an initial filter: the filter is a format -
               if it evaluates to zero, the item in the list is not
               shown, otherwise it is shown.  If a filter would lead to
               an empty list, it is ignored.  -F specifies the format for
       ...
       list-buffers [-r] [-F format] [-f filter] [-O sort-order]
               ...
               is true are shown.  See the “FORMATS” section.  -O
               specifies the sort order: one of ‘name’, ‘size’, or
               ‘creation’ (time).  -r reverses the sort order.
       ...
       paste-buffer [-dprS] [-b buffer-name] [-s separator] [-t
               target-pane]
                     (alias: pasteb)
               Insert the contents of a paste buffer into the specified
               pane.  If not specified, paste into the current one.  By
               default, control characters are sanitized with vis(3); -S
               disables this.  With -d, also delete the paste buffer.

               When output, any linefeed (LF) characters in the paste
               buffer are replaced with a separator, by default carriage
               return (CR).  A custom separator may be specified using
               the -s flag.  The -r flag means to do no replacement
               (equivalent to a separator of LF).  If -p is specified,
...
## TERMINFO EXTENSIONS         [top](#top_of_page)
...
```
       ...
       Spb     Set the state and progress for the OSC9;4 progress bar.
...
```
       ...
       ⟨https://github.com/tmux/tmux.git⟩ on 2026-05-24.  (At that time,
...
[logind.conf(5)](../man5/logind.conf.5.html),
[user\_caps(5)](../man5/user_caps.5.html),
