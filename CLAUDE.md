...
> **未リリース差分（2026-07-13）**: 公開面は `diagnostics` を加えた計 10 ツールへ更新中。これは factory 向け read-only の機械可読診断で、version / MCP readiness / PTY 一覧要約 / optional vendor launcher 状態だけを返し、path・env・token・コマンド本文・PTY出力・raw log・認証状態は返さない。直下の「計 9 ツール」記述は v0.12.1 リリース時点の履歴で、この未リリース差分では 10 が正である。
...
- **実装は Node/TS**（要件: Node>=18 + tmux）。`src/index.ts`（`@modelcontextprotocol/sdk` で 10 ツール公開〔PTY 6＋エージェント起動 3＋read-only diagnostics 1〕・stdio）/ `src/core.ts`（tmux 制御・出力削減・完了検出4層・安全ガード・rtk委譲・last-cmd。**stdout に出さない**＝通信を汚さない）/ `src/rtk.ts`（コマンド別 reducer）。`npm run build` → `dist/`。`package.json` の bin=`aiterm-mcp`→`dist/index.js`。
