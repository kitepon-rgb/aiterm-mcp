...
> **未リリース差分（2026-07-13）**: 公開面は `diagnostics` を加えた計 10 ツールへ更新中。これは factory 向け read-only の機械可読診断で、version / MCP readiness / PTY 一覧要約 / bounded runtime-error-store status / optional vendor launcher 状態だけを返し、path・env・token・コマンド本文・PTY出力・raw log・認証状態は返さない。あわせて製品所有の offline aggregate store と `aiterm-runtime-errors snapshot|ack|resolve|reopen` CLI を追加。schema-exact canonical dotagents config の `collection.enabled` が JSON boolean `true` の時だけ固定 code/template を集約し、raw exception/stderr/stack/prompt/PTY/transcript/event/path は API で拒否する。MCP側はbounded child processへ隔離、保存JSONはfingerprint再計算付きexact validation、lockはPID＋start identity＋token、POSIXはevery-read owner/mode、Windowsはcurrent SID-only DACL readback、coreはtyped owner errorで再計上を防ぐ。直下の「計 9 ツール」記述は v0.12.1 リリース時点の履歴で、この未リリース差分では 10 が正である。
