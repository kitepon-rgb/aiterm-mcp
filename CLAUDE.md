...
**最新は v0.7.1（2026-07-04）＝計 9 ツール**: PTY 6 ツール（`pty_open`/`pty_send`/`pty_read`/`pty_key`/`pty_close`/`pty_list`）＋**対話エージェント起動 3 ツール**（`codex_agent`/`grok_agent`/`composer_agent`、v0.7.0〜。各ベンダー CLI の対話 TUI を永続 PTY 内に起動して session_id を返し、以後 pty_read/pty_send で操作。前提検証は effort→bin→cwd の順で session 作成前・失敗時の残骸ゼロ保証は v0.7.1）。経緯: v0.5.0-0.6.0 で非対話 `delegate` を実装→対話パラダイム不整合のため v0.7.0 で撤去（非対話委譲は codex-sidecar の責務）。リリースは **tag `v*` push で CI が npm publish --provenance、GitHub Release 公開で registry.yml が MCP Registry 再登録**。回帰テストは計 **128 件**（2026-07-05 の全域監査スイープでバグ修正・回帰追加。詳細は `docs/03_audit-sweep-2026-07.md`）。

**2026-07-05 スイープの主な挙動変更（利用時に留意）**: ①`pty_read` の `until` は**既定でリテラル部分一致**（正規表現は `until_regex:true` でオプトイン）。②`pty_send(mark:true)` は `pty_read(wait:true)` が **until 無しでも sentinel を自動検出**して完了確定する（ネスト・非シェル前面でも効く。mark は POSIX シェル前提＝fish/csh は拒否）。③`read rtk:true` の pytest は**収集エラーを緑/無害に偽装しない**。④破壊ゲートに `rm -rf ./*`・引用符付き root・`..`・`./` を追加。⑤`screen+wait`・`full+lines` が機能化。⑥エージェント起動フラグ・モデル ID は実 CLI で裏取り済み（`codex 'prompt'`／`-c model_reasoning_effort=`／grok `--model grok-build|grok-composer-2.5-fast --effort <lvl>`）。

次の 1 段落は v0.4.1 時点の記述（歴史・検証実績として保持。ツール数 6・テスト数 93 は当時の値）:

**Node/TypeScript の npm パッケージ**として実装し **npm に公開済み**（`aiterm-mcp@0.4.1`、2026-06-08 公開・provenance 付き（0.4.0〜初版 0.1.0 は 2026-06-02 公開）。0.4.1 は発見性メタ／README 刷新のみでコード不変（公式 MCP Registry `io.github.kitepon-rgb/aiterm-mcp`・mcp.so に掲載、`server.json`＋`.github/workflows/registry.yml`（OIDC）で再登録自動化、発見性/告知の運用は `docs/PROMOTION.md`、アバターは `.github/avatar.{svg,png}`）。リポジトリは `github.com/kitepon-rgb/aiterm-mcp`）。`npx -y aiterm-mcp`（または `npm i -g aiterm-mcp` 後に `aiterm-mcp`）で起動でき、**絶対パス・venv 依存はゼロ**。WSL2 の Claude Code に**ユーザースコープ（global）**で登録済み（`~/.claude.json`、command=`aiterm-mcp`＝npm 公開版を `npm i -g` で導入。実機で公開版の起動・6 ツール・再起動跨ぎ永続を確認済み）。`pty_open`/`pty_send`/`pty_read`/`pty_key`/`pty_close`/`pty_list` の 6 ツール。**Windows ネイティブ／macOS にも対応**（Windows は 2026-06-02、macOS は 2026-06-02 に実機適応。Windows は tmux が無いため全 tmux 呼び出しを `wsl.exe -e tmux` 経由へ橋渡し。Windows 実機で `npm i -g`＋6 ツール公開＋PowerShell 入れ子＋回帰テスト（native Windows は POSIX 専用 1 件を skip＝92 pass/1 skip）を確認。macOS は実機（Apple Silicon・Homebrew tmux 3.6b）で回帰 93/93＋ライブ E2E（open/send/quiescence 完了検出/mark+until/screen/list/close）を検証。macOS 固有対応として `src/core.ts` に tmux 解決層を追加: `AITERM_TMUX` 上書き→PATH→Homebrew 既定（`/opt/homebrew/bin`・`/usr/local/bin`）の順で解決し、GUI 起動で Homebrew が PATH 外でも発見（使用時は stderr へ告知）、不在時は brew 導入を促す明確診断（旧: 空 stderr の握り潰し）、bash 3.2 の zsh 移行バナーを darwin 限定で抑止。WSL2/Linux も 93/93 で不変。0.4.0 でネスト中＋until無しの完了判定を早期返却化＝`is_complete=False via nested`、空費 timeout を解消）。ローカル / ネスト(`ssh 192.168.1.2`) / 再起動跨ぎ永続 / 出力削減を実機検証済み。設計は `docs/01_design-plan.md`、MCP 化の計画は `docs/02_mcp-plan.md`。
...
- `test/` — **Node 版の回帰テスト**（`node:test`、`npm test` で build→実行・tmux 必須）。`test/rtk.test.mjs`（pytest は実機 rtk 0.42.0 採取の golden と一致を `test/fixtures/pytest/*` で固定。`proj_ra` の `FAILED` 要約行のみ理由全文保持＝意図的に rtk 非一致／grep・git・filters は Python プロトタイプ生成の `test/fixtures/reducers.json` で固定／classify・truncate・stripShellFrame）、`test/core-pure.test.mjs`（stripControl・reduceOutput）、`test/core-readoutput.test.mjs`（readOutput の full/range/lines/offset/raw/rtk を tmux 非依存で）、`test/core-tmux.test.mjs`（破壊ゲート10種・サニタイズ・sendKey・完了検出。専用ソケットで隔離）、`test/smoke.test.mjs`（stdout が JSON-RPC のみ・9 ツール）、`test/core-space-path.test.mjs`（空白入り一時パスでも pipe-pane が出力を捕捉＝tmux 内部 /bin/sh のクオート回帰）。クロスプラットフォーム対応に伴い session 名検証（トラバーサル/インジェクション遮断・全入口）・`toWslPath`・offset クランプの回帰も追加。`test/core-resolve.test.mjs`（POSIX の tmux 解決負経路: `AITERM_TMUX` 不正時に空 stderr でなく明確な code2＝Windows は WSL ブリッジ経由のため skip）も追加。`test/core-agent.test.mjs`（openAgent の前提検証・残骸ゼロ保証 4 本・CODEX_BIN 偽装で CLI 未導入環境でも実行、v0.7.1）で計 **128 件**（2026-07-05 の全域監査スイープで各所に回帰追加。POSIX は全実行、native Windows は POSIX 専用の一部を skip）。CI は `.github/workflows/ci.yml`（**ubuntu-latest と macos-latest** の Node 18/20/22 で build+test、tag で publish --provenance。**windows-latest（Node 20/22）を非ブロッキング（continue-on-error）で追加＝純粋層を検証**。WSL ブリッジ統合経路は手動 Windows 検証ゲートに残す）。
