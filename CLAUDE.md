...
**Node/TypeScript の npm パッケージ**として実装し **npm に公開済み**（`aiterm-mcp@0.1.0`、2026-06-02 公開・provenance 付き。リポジトリは `github.com/kitepon-rgb/aiterm-mcp`）。`npx -y aiterm-mcp`（または `npm i -g aiterm-mcp` 後に `aiterm-mcp`）で起動でき、**絶対パス・venv 依存はゼロ**。WSL2 の Claude Code に**ユーザースコープ（global）**で登録済み（`~/.claude.json`、command=`aiterm-mcp`＝npm 公開版を `npm i -g` で導入。実機で公開版の起動・6 ツール・再起動跨ぎ永続を確認済み）。`pty_open`/`pty_send`/`pty_read`/`pty_key`/`pty_close`/`pty_list` の 6 ツール。**Windows ネイティブ／macOS にも対応**（2026-06-02。Windows は tmux が無いため全 tmux 呼び出しを `wsl.exe -e tmux` 経由へ橋渡し。Windows 実機で `npm i -g`＋6 ツール公開＋PowerShell 入れ子＋回帰テスト 91/91・skip 0 を確認。WSL2 も 91/91 で不変）。ローカル / ネスト(`ssh 192.168.1.2`) / 再起動跨ぎ永続 / 出力削減を実機検証済み。設計は `docs/ai-terminal-design-plan.md`、MCP 化の計画は `docs/mcp-server-plan.md`。
