...
> refuter反証（1st pass）: P0なし、P1 2件（復旧案内の--cursor欠落＝race再導入・claude_turn issueが観測を破棄する契約矛盾）を検出し還流済み。full regression 297/297（新規6件込み）。additive schema追加のため0.18.0へminor bump（package.json／package-lock／server.json同期）。公開証跡は本節末尾に追記する。
>
