...
6. 適用対象は POSIX（Linux / WSL2 / macOS）と **Windows ネイティブ**。〔2026-08-16改訂〕Windows は WSL 橋を全廃し、tmux CLI 互換のネイティブ実装 **psmux**（`-L` namespace 隔離・`AITERM_PSMUX` で上書き可）を直接叩く。pane shell は Git for Windows の `bash.exe`（`AITERM_BASH` で上書き可。System32 の `bash.exe` は WSL launcher のため使わない）。pipe-pane ログは Windows パス直渡し（psmux が `cat >> path` を in-process の直接ファイル sink として処理する）。旧 WSL 橋（`wsl.exe -e tmux`・`/mnt/c` 変換・interop anchor）は e3f5fc8 で撤去した。この置換は psmux ≥ 3.3.8 を前提とする——pipe-pane ファイル sink・paste 逐語 wire・前面 `#{pane_current_command}` の3修正（quolu 名義で upstream 貢献・psmux/psmux#577 として merge 済み）が入った最初の公開版。
7. （2026-07-18 追記・v0.18.0）agent TUI への prompt 投入は「submit の成立」を保証できない（非ブロック dispatch の原理的帰結。実被弾: 子 harness の startup ハング中に prompt が composer へ未 submit で座礁）。対処は D（安全性ガード）の一部として3層で確定: ①投入は tmux bracketed paste（pane negotiation）で原子化し、キー解釈由来の文字化け・Enter 取り落としを抑える ②ready gate は busy 表示中（esc to interrupt・実機根拠のある harness のみ）を ready と数えない ③投入後に composer 残存の**陽性証拠だけ**を有界観測し receipt（`submit_residue`）で報告する——auto-retry・例外化はしない（観測と操作の分離。false は成立の保証ではないと契約に明記）。
