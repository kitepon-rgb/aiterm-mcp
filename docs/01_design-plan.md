...
- **起動中agentのmodel／effort変更（2026-08-12・v0.24.0、2026-08-13・v0.24.3追補）**: `agent_configure`は既存のCodex／Claude sessionへvendor標準の`/model`（Claudeは加えて`/effort`）を送り、PTY・vendor session・会話contextを作り直さず設定だけを変更する。Codexのready判定は、起動直後の`OpenAI Codex` headerまたは長寿命画面に常駐するmodel／effort／任意`fast` footerと入力欄を使う。Codex v0.147の`medium fast ·`もfrontendであり、caller側の再描画・再試行・再起動は不要。Grok／Composerは対象外。公開受入は[27-agent-configure-release-plan.md](27-agent-configure-release-plan.md)、v0.24.3追補は[28-agent-env-vars-release-plan.md](28-agent-env-vars-release-plan.md)を正とする。
...
- **agent launch能力宣言（2026-08-02・source）**: `codex_agent`/`grok_agent`/`composer_agent`へ任意`write_scope`を追加した。指定値はlaunch receipt・per-launch metadata・`pty_list`へ保存する。ローカル`codex --help`で`--sandbox`の`read-only`値を確認し、Codexの`write_scope:"read-only"`だけは`--sandbox read-only`で実効書込み壁を作る。Grok/Composerはsandbox起動機構がなく、Codexのパス説明もallowlistフラグを持たないため、`write_scope_enforcement:"declaration_only_unsupported"`で能力壁未対応を明示する。省略時は旧argv・receipt・metadata表記を不変とする。
