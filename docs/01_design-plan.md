...
- **起動中agentのmodel／effort変更（2026-08-12）**: `agent_configure`は既存のCodex／Claude sessionへvendor標準の`/model`（Claudeは加えて`/effort`）を送り、PTY・vendor session・会話contextを作り直さず設定だけを変更する。Grok／Composerは対象外。
