...
9. （2026-08-15追記・オーナー裁定）Windows hostのvendor agentは**Windowsネイティブ実体で完結させ、WSL2側へvendor状態を持ち込まない**。grok/composerはWindows nativeの`grok.exe`だけを起動し（非nativeはsession作成前に明示エラー）、auth・session記録・transcript読取をすべてWindows側`~/.grok`で閉じる。〔2026-08-16改訂〕基盤自体がnative psmuxへ移行したため（決定6）、この裁定の後段にあったWSL interop anchor・`WSLENV`搬送はtransportごと撤去済み。native強制の原則だけが現行契約として残る。
...
- **agent launcher通常環境共有（2026-08-04・v0.22.0）**: `claude_agent`／`codex_agent`／`grok_agent`／`composer_agent`は直接CLI起動と同じ通常`HOME`、vendor home、project/user/local設定、MCP、plugin、skill、permission、trust、memory、historyを使う。aitermは環境をsnapshot・filter・置換せず、launch相関ID、完了event/cursor、bounded result、cleanup metadataだけを所有する。子へ`role=subagent`、親session、delegation depth、lineage、`delegation_allowed=true`を注入し、孫以降への再委譲は許可、固定depth capは設けない。4vendor depth 1とClaude nested depth 2を実smoke済み。Grok/Composerは共有MCPの`mcp_init_completed`後かつ入力欄readyまでpromptを送らない。Decisionは[ADR 0025](adr/0025-shared-agent-environment-and-lineage.md)を正とする。
- **launcher指定環境変数の現在値継承（2026-08-13・v0.24.3）**: tmux serverはMCP processより長寿命で、shellが継承するserver環境だけでは後から割り当てた席identity／workflow値を失う。4 launcher共通の任意`env_vars`は変数名だけを受け、起動時の現在のMCP processから存在する値を読み、shell quoteした起動単位prefixとしてvendor commandへ加える。未設定名は省略、不正なshell変数名はsession作成前に拒否する。全環境copy、name/value map、tmux server環境の更新・再起動、retry／fallbackは行わない。値はtool引数には入らないがPTY起動コマンドと`.lastcmd`へ到達するためsecret transportではない。既存tmux server条件のfocused regressionとPeertable実席9席で確認。設計・公開受入は[28-agent-env-vars-release-plan.md](28-agent-env-vars-release-plan.md)、環境Decisionは[ADR 0025](adr/0025-shared-agent-environment-and-lineage.md)を正とする。
...
- **Grok／Composer起動modelのfail-loud照合（2026-08-13・v0.25.0 source／v0.25.1公開）**: 明示`model`とComposer既定`grok-composer-2.5-fast`はPTY作成前に同じGrok CLIの`grok models`出力へ照合する。catalog取得失敗、形式不正、model不在は明示エラーで、cache・retry・別model fallbackは行わない。現行vendor catalogにComposer modelが無ければComposer既定起動は利用不可として正しく失敗する。Grokの省略時既定`grok-4.5`だけは従来の固定既定を保つ。Decisionは[ADR 0029](adr/0029-grok-composer-agent-parity-scope.md)、公開受入は[ADR 0030](adr/0030-release-0.25.1-acceptance.md)を正とする。
- **Grok設定変更の常駐footer確認（2026-08-14・v0.25.2）**: Grok Build 1.0.3は`/model`成功通知を次の再描画で消すことがある。変更前には無かった要求model／effortが、vendor自身の常駐footerへ変更後に現れた場合も完了として受理する。既存の明示エラーと成功通知を第一の判定に保ち、変更前から同じfooterだった場合は成功へ丸めない。PeertableのGrok 4.6実席で4.6→4.5→4.6を同一sessionのまま確認し、caller側のretryや再起動は追加しない。公開受入は[ADR 0031](adr/0031-release-0.25.2-acceptance.md)を正とする。
