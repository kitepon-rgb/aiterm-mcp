...
- **起動中agentのmodel／effort変更（2026-08-12・v0.24.0）**: `agent_configure`は既存のCodex／Claude sessionへvendor標準の`/model`（Claudeは加えて`/effort`）を送り、PTY・vendor session・会話contextを作り直さず設定だけを変更する。Codexのready判定は、起動直後の`OpenAI Codex` headerまたは長寿命画面に常駐するmodel／effort footerと入力欄を使う。Grok／Composerは対象外。公開受入は[27-agent-configure-release-plan.md](27-agent-configure-release-plan.md)を正とする。
