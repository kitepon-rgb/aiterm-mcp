...
9. （2026-08-15追記・オーナー裁定）Windows hostのvendor agentは**Windowsネイティブ実体で完結させ、WSL2側へvendor状態を持ち込まない**。WSLはtmux transport（§9.6）とinterop anchorだけの役に限定する。grok/composerはWindows nativeの`grok.exe`だけを起動し（非nativeはsession作成前に明示エラー）、auth・session記録・transcript読取をすべてWindows側`~/.grok`で閉じる。pane内からの`.exe`起動は、paneが継承した`WSL_INTEROP`の起動元sessionが死んでいるとbinfmt interopが`accept4=110`で失敗するため、aitermが長寿命interop anchor（sleepする`wsl.exe` process）を所有して生きたsocketを起動envへ供給する。WSL側envはinterop先のWindows processへ既定では渡らないため、注入envは`WSLENV`（`/w`）で明示的に運ぶ（いずれも実測 2026-08-15・WSL 2.6.1）。
