...
- **起動中agentのmodel／effort変更（2026-08-12・v0.24.0、2026-08-13・v0.24.3／v0.25.0追補、2026-08-14追補）**: `agent_configure`は既存のClaude／Codex／Grok／Composer sessionへvendor標準操作を送り、PTY・vendor session・会話contextを作り直さず設定だけを変更する。Codex／Claudeは既存経路、Grok／Composerはlive catalog照合後の`/model <model> [effort]`または`/effort <effort>`を使う。Codexのready判定は、起動直後の`OpenAI Codex` headerまたは長寿命画面に常駐するmodel／effort／任意`fast` footerと入力欄を使う。Grok／Composerは構造化`mcp_init_completed`と入力欄readyを共通前提にする。Grokの変更完了は追加された成功通知、または変更前には無かった要求model／effortが常駐footerへ現れた事実で確認する。公開受入は[27-agent-configure-release-plan.md](27-agent-configure-release-plan.md)、v0.24.3追補は[28-agent-env-vars-release-plan.md](28-agent-env-vars-release-plan.md)、Grok／Composer追補は[29-grok-composer-agent-parity-plan.md](29-grok-composer-agent-parity-plan.md)を正とする。
...
- 残課題: 状態追跡(A)・ネスト層の完了判定（ssh 中は前面コマンドが ssh でシェル復帰判定が効かない＝`--until` で代替）、`ls`/`git diff` の自前版（再実行型ゆえローカルは委譲がカバー）、フィルタ拡充。Windows専用分岐（`wsl.exe`橋渡し・preflight・settle）を含む現行最終CIは、self-hostedのmacOS native・Linux native・Windows native・WSL2で同じfull `npm test`を実行する。GitHub-hosted Windowsの純粋層や手動検証を最終CIの代用にしない。
