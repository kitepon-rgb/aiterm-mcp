...
- **Windowsを含むwaiter process境界（2026-08-29・v0.29.7）**: npmのhuman-facing binはplatform別shimで、Windowsでは`aiterm-wait.ps1`になる。`wait_command`文字列だけを返すと、callerが`.ps1`を直接process executableへ渡して`ERROR_BAD_EXE_FORMAT`となり、利用側のhost判定・wrapperへ逃げる。`agent_launch`とagent `pty_send`のreceiptへ`wait_process={executable,args}`を追加し、現在稼働中の`process.execPath`と同梱`aiterm-wait-cli.js`を分離して返す。machine callerはこの境界をそのまま別process APIへ渡すだけとし、`wait_command`は人間向け互換表示に限定する。Windows backendはpsmux、対話shellはPowerShell 7のままで、waiter outcome／state正本は変えない。Decisionは[ADR 0046](adr/0046-platform-native-wait-process-boundary.md)。
