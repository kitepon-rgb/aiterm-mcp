...
- **Grok設定変更の常駐footer確認（2026-08-14・v0.25.2）**: Grok Build 1.0.3は`/model`成功通知を次の再描画で消すことがある。変更前には無かった要求model／effortが、vendor自身の常駐footerへ変更後に現れた場合も完了として受理する。既存の明示エラーと成功通知を第一の判定に保ち、変更前から同じfooterだった場合は成功へ丸めない。PeertableのGrok 4.6実席で4.6→4.5→4.6を同一sessionのまま確認し、caller側のretryや再起動は追加しない。
