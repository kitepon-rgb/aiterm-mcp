...
6. 適用対象は POSIX（Linux / WSL2 / macOS）と **Windows ネイティブ**。〔2026-08-25改訂〕Windows は WSL 橋を全廃し、tmux CLI 互換のネイティブ実装 **psmux**（`-L` namespace 隔離・`AITERM_PSMUX` で上書き可）をterminal/session multiplexer backendとして直接叩く。psmuxはshellではない。公開`pty_open`の既定shellはPowerShell 7（`pwsh.exe`）だけで、検出実体自身の`PSEdition=Core`／major 7以上を確認し、5.1／PowerShell 6／`cmd.exe`へfallbackしない。Git for Windows の`bash.exe`（`AITERM_BASH`で上書き可。System32の`bash.exe`はWSL launcher）はharness launcherが内部で明示するshellとして残す。pipe-paneログはWindowsパス直渡し。旧WSL橋はe3f5fc8で撤去済み。psmux ≥ 3.3.8を前提とし、他製品はpsmuxへ直接依存せずAiterm公開APIを使う。
7. （2026-07-18 追記・v0.18.0、2026-08-29 Windows改訂）agent TUI への prompt 投入は「submit の成立」を保証できない（非ブロック dispatch の原理的帰結。実被弾: 子 harness の startup ハング中に prompt が composer へ未 submit で座礁）。対処は D（安全性ガード）の一部として3層で確定: ①POSIX tmuxは`paste-buffer -p`のpane negotiationで原子化する。Windows psmux 3.3.8は実在bufferへ`no buffer`を返し、反復CLI `send-keys`もConPTY queueを越えて交差するため、session serverの認証済みTCP commandへ256byte単位の`SendBytes`をpacingして投入し、server処理完了と最終drainまでsession lockを保持する。ready gate通過済みagent TUIのwrapperも同じbyte streamへ含める ②ready gate は busy 表示中（esc to interrupt・実機根拠のある harness のみ）を ready と数えない ③投入後に composer 残存の**陽性証拠だけ**を有界観測し receipt（`submit_residue`）で報告する——auto-retry・例外化はしない（観測と操作の分離。false は成立の保証ではないと契約に明記）。
...
9. （2026-08-15追記・オーナー裁定）Windows hostのharness agentは**Windowsネイティブ実体で完結させ、WSL2側へharness状態を持ち込まない**。grok/composerはWindows nativeの`grok.exe`だけを起動し（非nativeはsession作成前に明示エラー）、auth・session記録・transcript読取をすべてWindows側`~/.grok`で閉じる。〔2026-08-16改訂〕基盤自体がnative psmuxへ移行したため（決定6）、この裁定の後段にあったWSL interop anchor・`WSLENV`搬送はtransportごと撤去済み。native強制の原則だけが現行契約として残る。
10. （2026-08-19追記・オーナー裁定）**共有/tmpの敵対的同居主体を前提とした安全設備は全プラットフォームで撤去**する（agent state系のsymlink・hard link・owner uid比較・mode bit検査・`O_NOFOLLOW`）。対応OSの既定配置（`XDG_RUNTIME_DIR`／per-userの`os.tmpdir()`）では前提が成立しないため。残すのは短書き込み検出（書込整合性）、`dev`/`ino`同一性（operation相関の正しさ）、size上限（parse入力）、作成時0o600/0o700（既定値）だけ。Grok auth正本の検証（資格情報・脅威モデルが別）とsend/wait lock（並行制御の正しさ）は撤去対象外。あわせて、受入が「使える」と判定したbinはcontrol command（`claude auth status`等）でも実行できなければならない——Windowsでshebang scriptをGit Bash経由・`.cmd`/`.bat`をshell経由で実行する整合性修理を入れた。この裁定の実行で、テスト覆域のWindows回復（skip 155→45・fail 0）と、grok-stop-hookがWindowsで即failしGrok/Composer完了eventが書かれない実バグの根治が同時に確定した（工程正本はdocs/31）。
11. （2026-08-24追記）agent起動の正規APIは`agent_launch({harness, model?, ...})`へ統一し、**harnessとmodelを別軸**にする。harnessはagent loop・認証・hook・session・transcriptを所有する実行基盤（Claude Code／Codex CLI／Grok CLI／Cursor Agent CLI）、modelはその上で選ぶ推論モデルである。CursorがGPT／Claude／Grokを選んでもCursor harness、ComposerはGrok CLI上のmodel presetとする。旧4 launcherは移行用aliasとして残す。Cursor固有実装は通常`~/.cursor`を共有し、launch IDで通常agent transcriptをbindする。完了は末尾`turn_ended`、`event_cursor`はfollow-up書換えに耐えるuser turn数。詳細はADR 0038。
12. （2026-08-30追記）Windows nativeの隔離単位はpsmux namespaceとmanaged state rootで同じ`TMPDIR`を使う。Nodeの`os.tmpdir()`はWindowsで`TMPDIR`を無視して`TEMP`を返すため、PTYだけ隔離したtest processの`killAll()`が本番`%TEMP%\aiterm-mcp-0\agents`を削除し、生存sessionから完了相関だけを失わせた。Windowsの`runtimeStateBase()`は`TMPDIR`設定時だけそれを最優先し、未設定の通常serverは従来どおり`%TEMP%`を使う。詳細はADR 0047。
...
- **B**: quiescence は固定 ms でなく「出力静止(≈2ポーリング) ∧ `pane_current_command` がシェルに復帰」で判定。`--until`(sentinel/prompt) と timeout を併用（4層）。`until`／`mark`指定時はその証拠をquiescenceより優先し、shell builtin処理中の一時静止を完了へ昇格しない。
...
- **クロスプラットフォーム（2026-06-02・履歴。2026-08-16に決定6で全面置換）**: 当時は`src/core.ts`のOS分岐からWindowsの全tmux呼出を`wsl.exe -e tmux …`へ橋渡しした。WSL socket、`/mnt/c`ログ変換、`settleWinLog`を含むこのtransportはe3f5fc8で全廃済みで、現行Windows nativeは`src/tmux-runtime.ts`／`src/agent-resolver.ts`が所有するpsmux 3.3.8以上＋Git Bash経路だけを使う。セッション名の`^[A-Za-z0-9_-]{1,64}$`検証と、POSIX pipe-paneパスの単一引用符＋`'\''`エスケープは現行にも継承する。
...
- **長文PTY送信と明示完了証拠の全OS修理（2026-08-13・`aiterm-mcp@0.25.1`）**: Linux nativeで6,000文字の入力がtmux成功のまま5,487文字へ欠落し、WSL2でも同じ回帰を確認した。macOS限定だった分割を全OSのUTF-8安全な256byte chunk＋chunk間10ms drainへ広げる。`until`／`mark`指定中はshell前面のquiescenceで早期完了せず、marker／sentinel一致、dead、timeoutだけで決着する。6,000文字、遅延mark、通常quiescentのfocused回帰をmacOS／Linux／WSL2で固定する。
- **PowerShell mark（2026-08-23・`aiterm-mcp@0.27.9`）**: Windows nativeで前面が`powershell`／`pwsh`の時はPOSIX用`printf`を連結せず、PowerShell構文でsentinelを実行時生成する。PowerShellの`$?`は真偽なので成功`rc=0`／失敗`rc=1`とする。command echoは`rc={0}`を保持し、数字アンカー検出が実出力前に発火しない。POSIX形式は不変、fish/csh/tcshは送信前に明示拒否する。
...
- **起動中agentのmodel／effort変更（2026-08-12・v0.24.0、2026-08-13・v0.24.3／v0.25.0追補、2026-08-14／2026-08-24追補）**: `agent_configure`は既存のClaude／Codex／Grok／Composer／Cursor sessionへharness標準操作を送り、PTY・harness session・会話contextを作り直さず設定だけを変更する。Codex／Claudeは既存経路、Grok／Composerはlive catalog照合後の`/model <model> [effort]`または`/effort <effort>`を使う。Cursorは`model-effort` IDをlive catalogへ照合し、標準`/model` pickerのparameter editorで同一sessionを変更する。Codexのready判定は、起動直後の`OpenAI Codex` headerまたは長寿命画面に常駐するmodel／effort／任意`fast` footerと入力欄を使う。Grok／Composerは構造化`mcp_init_completed`と入力欄readyを共通前提にする。Grokの変更完了は追加された成功通知、または変更前には無かった要求model／effortが常駐footerへ現れた事実で確認する。Cursor追補と標準`agent_launch`は[ADR 0038](../adr/0038-harness-launch-api-and-cursor-agent-cli.md)、公開受入は[ADR 0039](../adr/0039-release-0.28.0-acceptance.md)を正とする。旧経路の公開受入は[27-agent-configure-release-plan.md](27-agent-configure-release-plan.md)、v0.24.3追補は[28-agent-env-vars-release-plan.md](28-agent-env-vars-release-plan.md)、Grok／Composer追補は[29-grok-composer-agent-parity-plan.md](29-grok-composer-agent-parity-plan.md)を正とする。
...
- **agent launcher通常環境共有（2026-08-04・v0.22.0）**: `claude_agent`／`codex_agent`／`grok_agent`／`composer_agent`は直接CLI起動と同じ通常`HOME`、harness home、project/user/local設定、MCP、plugin、skill、permission、trust、memory、historyを使う。aitermは環境をsnapshot・filter・置換せず、launch相関ID、完了event/cursor、bounded result、cleanup metadataだけを所有する。子へ`role=subagent`、親session、delegation depth、lineage、`delegation_allowed=true`を注入し、孫以降への再委譲は許可、固定depth capは設けない。4harness depth 1とClaude nested depth 2を実smoke済み。Grok/Composerは共有MCPの`mcp_init_completed`後かつ入力欄readyまでpromptを送らない。Decisionは[ADR 0025](../adr/0025-shared-agent-environment-and-lineage.md)を正とする。
- **launcher指定環境変数の現在値継承（2026-08-13・v0.24.3）**: tmux serverはMCP processより長寿命で、shellが継承するserver環境だけでは後から割り当てた席identity／workflow値を失う。4 launcher共通の任意`env_vars`は変数名だけを受け、起動時の現在のMCP processから存在する値を読み、shell quoteした起動単位prefixとしてharness commandへ加える。未設定名は省略、不正なshell変数名はsession作成前に拒否する。全環境copy、name/value map、tmux server環境の更新・再起動、retry／fallbackは行わない。値はtool引数には入らないがPTY起動コマンドと`.lastcmd`へ到達するためsecret transportではない。既存tmux server条件のfocused regressionとPeertable実席9席で確認。設計・公開受入は[28-agent-env-vars-release-plan.md](28-agent-env-vars-release-plan.md)、環境Decisionは[ADR 0025](../adr/0025-shared-agent-environment-and-lineage.md)を正とする。
- **Throughline portable fork（2026-08-04・v0.23.0）**: 4 launcher共通の任意引数`throughline_source_session`を追加。指定時はPTY作成前にローカルの`throughline handoff-context --session <id> --json`を1回だけ実行し、schema/status/sessionId/contextを検証して、返されたcontext＋固定区切り＋必須の新ミッション`prompt`を初回promptにする。Throughline側は同じbudgeted resume builderをread-only DBへ適用し、元sessionのrow所属・`merged_into`・batonを変更しない。`launch_operation_id`とは併用不可。不在、非zero、不正schema、空contextはclean launchへfallbackせず残骸ゼロで失敗する。省略時は従来のclean launchで、retry/cache/timeout/version probeや新MCP toolは設けない。設計・受入条件は[26-throughline-portable-fork-plan.md](26-throughline-portable-fork-plan.md)を正とする。
- **旧managed環境契約の読み方**: 以下のmanaged home／fake `HOME`／設定snapshot／隔離記述は各版の歴史的受入記録であり、v0.22.0の現行挙動ではない。文中の「現在」はその記録時点を指し、現行契約は直前項とADR 0025が上書きする。
- **agent_done（2026-07-07・`aiterm-mcp@0.9.0`/`0.9.1` 公開）**: `*_agent(agent_done:true)` と `pty_send(wait:"agent_done")` を追加。公開ツール数は増やさず、harness Stop hook を完了境界に使う。Codex は managed `CODEX_HOME` を launch ごとに作り、通常 `~/.codex/hooks.json` は触らず、Stop chain を aiterm が単独所有する。v0.9.1 で Codex managed home は allowlist 化し、通常 Codex home からは `auth.json` だけを symlink、`config.toml` は managed home へ private copy、その他の state/cache/session entry は共有しない。Grok/Composer は managed `GROK_HOME` + fake `HOME` + `HOME/.grok -> GROK_HOME` symlink で compat hook/plugin 混入を抑える。OAuth は通常 Grok home の `auth.json` と `auth.json.lock` をセットで共有した（0.9.1当時）。この方式は2026-07-14に廃止し、現在は検証済み通常auth正本を`GROK_AUTH_PATH`でharnessへ渡し、managed homeへauth/lockを置かない。event file は secure state root 配下の `<session>.<launch_id>.events.jsonl`、現在ターン境界は送信直前 EOF。Codex/Grok/Composer は実 smoke 済み。Grok/Composer は `grok login` 後に Grok TUI・Composer TUI・通常 headless `grok -p` の並行 smokeも通過。同一 cwd の Codex/Grok/Composer 並列 agent_done smoke、普通PTYの Python REPL smoke、security/schema/screen-settle/release-metadata 回帰も通過。MCP stdio server を実起動した JSON-RPC `tools/call` 経由 smoke でも、3 vendor同時 agent_done と普通PTY Python REPL が通過。v0.9.1 の npm publish、Official MCP Registry 登録、global install smoke、GitHub Release 反映まで完了済み。
- **Codex完了正本のrollout移行（2026-08-03・v0.21.1）**: 実運用で長寿命MCP serverが`process.execPath=/opt/homebrew/Cellar/node/<旧版>/bin/node`をCodex `hooks.json`へ焼き付け、Homebrew更新後に実体が消えてStop hookが`exit 127`、`aiterm-wait`が600秒timeoutを反復した。Codex自身はhookより先にmanaged `CODEX_HOME`のroot rolloutへ`task_complete.turn_id`を永続化していたため、Codexの完了正本をこのharness記録へ移す。既存`event_cursor`はCodexだけdispatch直前のroot transcript byte境界を表し、`aiterm-wait`と`agent_transcript`が同じturnを読む。managed Codex homeへStop hookと`--dangerously-bypass-hook-trust`を作らず、未使用のCodex hook実装も配布対象から撤去する。buildは既存`dist/*.js`を先に消し、dirty workspaceでも廃止helperをnpm tarball／MCPBへ再混入させない。sub-agent rolloutはrootへ誤帰属せず、follow-upも毎回idle gate後に境界を切る。Claude/Grok hookは継承PATHの`node`を実行時解決し、版付きCellar実体を永続設定へ固定しない。不変Decisionは[ADR 0022](../adr/0022-codex-rollout-completion.md)を正とする。
...
- **managed Claude共有認証preflight（2026-08-01・v0.20.3）**: 新しいClaude TUIを作る前に同じCLIの`auth status --json`を5秒上限で検証し、exit 0・JSON object・`loggedIn:true`の三条件だけを起動可とする。未認証／malformed／失敗exit／timeoutはsession・metadata・event/resultを作らず明示失敗する。認証はvendor所有の通常credential storeを共有し、Aitermはcopy・symlink・lock・自動loginを行わない。managed sessionへのexact `/login`／`/logout`は通常dispatchとforce送信の双方で副作用前に拒否する。launcherは直列化せず、正常な共有認証で複数Claude sessionを並行・反復利用する。実Fable反復smokeで露呈したdone event→active marker削除間のraceは、同じmarkerより新しいresultと相関eventが揃った時だけ回収側が最大1秒cleanupをsettleする。設計と受入は[ADR 0020](../adr/0020-managed-claude-authentication-preflight.md)を正とする。
- **managed Claude user MCP継承（2026-08-04・v0.21.4）**: `--setting-sources ""`による通常hook隔離が、`~/.claude.json`のuser scope MCPまで消してfresh ClaudeからAIShellを不可視にしていた。top-level `mcpServers`だけをlaunch単位の0600 configへsnapshotし、公開`--mcp-config`で渡す。project／local MCPと通常settings／plugin／permissionは継承せず、破損configは残骸ゼロでfail loudする。設計は[ADR 0024](../adr/0024-managed-claude-user-mcp-inheritance.md)を正とする。
...
- **agent launch能力宣言（2026-08-02・source、2026-08-13・v0.25.0追補）**: `codex_agent`/`grok_agent`/`composer_agent`は任意`write_scope`を受け、launch receipt・per-launch metadata・`pty_list`へ保存する。3 launcherすべてで`write_scope:"read-only"`を`--sandbox read-only`へ落とし、実効書込み壁を作る。パス説明は同等のallowlist CLI引数がないため`write_scope_enforcement:"declaration_only_unsupported"`で宣言記録だけと明示する。省略時は旧argv・receipt・metadata表記を不変とする。
- **Grok／Composer起動modelのfail-loud照合（2026-08-13・v0.25.0 source／v0.25.1公開）**: 明示`model`とComposer既定`grok-composer-2.5-fast`はPTY作成前に同じGrok CLIの`grok models`出力へ照合する。catalog取得失敗、形式不正、model不在は明示エラーで、cache・retry・別model fallbackは行わない。現行harness catalogにComposer modelが無ければComposer既定起動は利用不可として正しく失敗する。Grokの省略時既定`grok-4.5`だけは従来の固定既定を保つ。Decisionは[ADR 0029](../adr/0029-grok-composer-agent-parity-scope.md)、公開受入は[ADR 0030](../adr/0030-release-0.25.1-acceptance.md)を正とする。
- **Grok設定変更の常駐footer確認（2026-08-14・v0.25.2）**: Grok Build 1.0.3は`/model`成功通知を次の再描画で消すことがある。変更前には無かった要求model／effortが、harness自身の常駐footerへ変更後に現れた場合も完了として受理する。既存の明示エラーと成功通知を第一の判定に保ち、変更前から同じfooterだった場合は成功へ丸めない。PeertableのGrok 4.6実席で4.6→4.5→4.6を同一sessionのまま確認し、caller側のretryや再起動は追加しない。公開受入は[ADR 0031](../adr/0031-release-0.25.2-acceptance.md)を正とする。
- **factory runtime error projection（2026-07-13・v0.12.2公開済み、2026-08-13・v0.24.2 queue期限修正）**: aiterm-mcp 自身が offline aggregate store と schema を所有し、schema-exact canonical dotagents factory config の `collection.enabled === true` だけを収集opt-inとする。core の PTY dependency・persistence・vendor launcher owner layer が固定 code だけを一度記録し、typed telemetry-owned error により上位vendor/cleanupで再計上しない。JSON-RPC adapter も重複記録しない。保存は固定 template、SHA-256 fingerprint、count/first/last、open/resolved、monotonic sequence に限定し、raw exception/stderr/stack/prompt/PTY/transcript/event/path/context は API で拒否する。保存JSONはexact field・固定定義・fingerprint再計算後のDTOだけを公開。MCP側の収集/diagnosticsはtimeout付きchild processへ隔離し、FIFO/停止filesystemでも本体を止めない。lockはPID＋process start identity＋tokenで、stale/ambiguousはABA回避のため明示削除する。bakery queueの期限はqueue総待ち時間でなく同じ先頭ownerの無進捗時間を測り、ticket進行ごとに更新する。通常pollはprocess生存確認だけとし、macOSで外部`ps`を使うprocess start identity照合はstall時だけ行う。POSIXはevery readでowner/mode、Windowsはcurrent SID-only DACLを再構築/readbackする。store failure は元の PTY/MCP result を変えず固定 stderr のみ。`aiterm-runtime-errors snapshot|ack|resolve|reopen` を dotagents consumer 入口とし、unacknowledged record は retention で削除しない。Windows native は path/DACL/timeout の純粋テストのみで、この変更では実機統合成功を主張しない。
...
- 残課題: 状態追跡(A)・ネスト層の完了判定（ssh 中は前面コマンドが ssh でシェル復帰判定が効かない＝`--until` で代替）、`ls`/`git diff` の自前版（再実行型ゆえローカルは委譲がカバー）、フィルタ拡充。現行最終CIは、POSIX tmuxとWindows native psmux／Git Bashの各分岐をself-hostedのmacOS native・Linux native・Windows native・WSL2で同じfull `npm test`により検証する。GitHub-hosted Windowsの純粋層や手動検証を最終CIの代用にしない。
...
- **Windowsを含むwaiter process境界（2026-08-29・v0.29.7）**: npmのhuman-facing binはplatform別shimで、Windowsでは`aiterm-wait.ps1`になる。`wait_command`文字列だけを返すと、callerが`.ps1`を直接process executableへ渡して`ERROR_BAD_EXE_FORMAT`となり、利用側のhost判定・wrapperへ逃げる。`agent_launch`とagent `pty_send`のreceiptへ`wait_process={executable,args,windows_start_process_argument_list}`を追加し、現在稼働中の`process.execPath`と同梱`aiterm-wait-cli.js`を分離して返す。true argv APIは`args`を使い、配列を空白結合するPowerShell 7 `Start-Process`だけは製品がWindows quoting済みの単一文字列を使う。`wait_command`は人間向け互換表示に限定する。Windows backendはpsmux、対話shellはPowerShell 7のままで、waiter outcome／state正本は変えない。Decisionは[ADR 0046](../adr/0046-platform-native-wait-process-boundary.md)。
