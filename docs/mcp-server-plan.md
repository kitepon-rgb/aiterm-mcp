...
> **Node/TS の npm パッケージ `aiterm-mcp` へ移行し npm 公開済み**（`aiterm-mcp@0.3.0`＝Windows/WSL2/macOS 全対応版（macOS は 0.3.0 で実機検証・tmux 解決層を追加）。初版 0.1.0 は 2026-06-02 公開。リポジトリ `kitepon-rgb/aiterm-mcp`。実装は `src/*.ts`、旧 Python は `prototype/python/`）。
