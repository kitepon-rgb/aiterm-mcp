...
- [x] Push the release commit and require the tag-triggered npm provenance CI gate to pass (`239e7e4`, CI `29245251184`).
- [x] After the owner H gate, create and push `v0.12.2`; verify npm publication, then create the GitHub Release so MCP Registry workflow `29245462227` registers the already-published version.
- [x] Verify the published package exposes ten MCP tools, factory diagnostics, and runtime snapshot; record npm, GitHub Release, and MCP Registry results in the canonical docs.
