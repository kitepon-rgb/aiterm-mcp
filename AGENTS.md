...
> **v0.29.8（2026-08-30・公開工程）**: Windows nativeの永続PTY backendはpsmux 3.3.8以上なのに、`pty_open`のMCP descriptionとREADME冒頭がtmux必須と誤案内していた欠陥を修理。公開metadataはPOSIX=tmux／Windows native=psmuxを明示し、Windowsにもtmuxを要求する旧説明へ戻らないMCP smokeを固定する。加えてClaude Code 2.1.251のworkspace trust menuが選択カーソルにも`❯`を使い、初回promptを`No, exit`へ誤送信して成功receiptを返す欠陥を修理する。画面内の最後の`❯`行がtrust選択肢ならcomposerと認めず、古いtrust表示の下に現在composerがある場合はreadyを維持する。さらにWindowsの`TMPDIR`隔離processがpsmux namespaceだけを分離し、managed stateは本番`TEMP`へ残して`killAll()`で生存Fableのmetadataを消していた欠陥を修理する。Windows state rootも`TMPDIR`最優先へ揃え、通常serverの`TMPDIR`未設定時は従来の`%TEMP%`を維持する。実画面の純粋回帰、prompt未送信統合回帰、foreign metadataを残すcross-process killAll回帰で固定。PowerShell 7対話shellは変更しない。
