...
- Codexのready判定は、起動直後の`OpenAI Codex` headerまたは長寿命画面に常駐するmodel／effort／任意`fast` footerと入力欄をfrontend根拠にします。caller側の再描画・再試行・再起動で補いません。
...
- 先に起動したtmux serverへ無いprocess-local値が必要な場合、4つのlauncherは`env_vars`へ環境変数名だけを受け、現在のMCP processから存在する値だけを起動コマンドへ加算します。全環境の暗黙copy、name/value引数、server再起動やfallbackは追加しません。値は`.lastcmd`と起動先vendorへ到達するため秘密転送には使いません。正本は`docs/28-agent-env-vars-release-plan.md`とADR 0025です。
